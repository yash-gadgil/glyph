...
export default function PixelEther({
  mouseForce = 20,
  cursorSize = 100,
  isViscous = false,
  viscous = 30,
  ...
  useEffect(() => {
    ...
    function makePaletteTexture(stops: (string | [number, number, number])[]): THREE.DataTexture {
      let arr: (string | [number, number, number])[];
...
`;
    ...
    float scaled = lenv * (N - 1.0);
    float floored = floor(scaled);
    float remainder = scaled - floored;
    
    float dithered = floored;
    if (remainder > threshold) dithered += 1.0;
    
    dithered = dithered / (N - 1.0);
    dithered = clamp(dithered, 0.0, 1.0);
    
    
    vec3 color1 = texture2D(palette, vec2(0.166666, 0.5)).rgb;
    vec3 color2 = texture2D(palette, vec2(0.833333, 0.5)).rgb;
    
    vec3 accent = mix(color1, color2, dithered);
    
    vec3 outRGB = mix(vec3(0.05), accent, lenv);
...
`;
  ...
  useEffect(() => {
    ...
    const newPaletteTex = makePaletteTexture(colors);
    const outputMesh = (webgl.output as any).output as THREE.Mesh;
    if (outputMesh && outputMesh.material) {
      const material = outputMesh.material as THREE.RawShaderMaterial;
      if (material.uniforms && material.uniforms.palette) {
        const oldTex = material.uniforms.palette.value;
        if (oldTex) oldTex.dispose();
        material.uniforms.palette.value = newPaletteTex;
      }
    }

    return () => {
      newPaletteTex.dispose();
    };
  }, [colors]);
